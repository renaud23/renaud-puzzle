...
import com.puzzle.generator.core.tools.ImageProvider;
import com.puzzle.generator.core.tools.TemplateMemoryManager;
...
//		}
	...
	public GeneratorPuzzle(Frame frame, PieceDescripteur descriptor,BufferedImage image,double scalePiece){
		this.frame = frame;
...
//		this.cleanDirectory(buildPath);
		...
		for(int j=0;j<this.frame.getHauteur();j++){
			...
			for(int i=0;i<this.frame.getLargeur();i++){
				...
				if(this.frame.getCell(i, j).getType() != Frame.case_vide){
...
//		FileUtils.savePng(this.tampon, "test/","y_"+y+"_"+x);
	...
	public BufferedImage makeVue(String pathImagesPieces,double scaleVue){
		...
		for(int j=0;j<this.frame.getHauteur();j++){
			for(int i=0;i<this.frame.getLargeur();i++){
				...
				if( (value&Frame.nord) == Frame.nord){
					y -= maxVar;
				}else if( (value&Frame.cote_nord) != Frame.cote_nord){
					y -= minVar;
				}
				if( (value&Frame.ouest) == Frame.ouest){
					x -= maxVar;
				}else if( (value&Frame.cote_ouest) != Frame.cote_ouest){
					x -= minVar;
				}
	...
	private Rectangle getRectangle(int i,int j){
		...
		if( (value&Frame.sud) == Frame.sud){
			h += maxVar;
		}else if( (value&Frame.cote_sud) != Frame.cote_sud){
			h += minVar;
		}
		if( (value&Frame.ouest) == Frame.ouest){
			w += maxVar;
		...
		if( (value&Frame.est) == Frame.est){
			w += maxVar;
		}else if( (value&Frame.cote_est) != Frame.cote_est){
			w += minVar;
		}
