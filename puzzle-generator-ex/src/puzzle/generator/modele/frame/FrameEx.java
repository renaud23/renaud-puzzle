...
public class FrameEx {
	 ...
	 * @param hauteur
	 */
	public FrameEx(int largeur,int hauteur){
				...
				if(selection.contains(id)){
					...
					if((j-1)>=0){
						...
						if(!selection.contains(idEx)){//System.out.println("yo");
							type = type|FrameEx.cote_nord;
							type = (type|FrameEx.nord)^FrameEx.nord;
						}
					}// if
					...
					if((j+1)< (this.hauteur)){
						...
						if(!selection.contains(idEx)){
							type = type|FrameEx.cote_sud;
							type = (type|FrameEx.sud)^FrameEx.sud;
						}
					}// if
					...
					if((i-1) >= 0){
						...
						if(!selection.contains(idEx)){
							type = type|FrameEx.cote_ouest;
							type = (type|FrameEx.ouest)^FrameEx.ouest;
						}
					}// if
					...
					if((i+1) <(this.largeur)){
						...
						if(!selection.contains(idEx)){
							type = type|FrameEx.cote_est;
							type = (type|FrameEx.est)^FrameEx.est;
						}
					}// if
	...
	private void moveLeft(){
		for(int i=0;i<(this.largeur-1);i++){
			for(int j=0;j<this.hauteur;j++){
				...
				next.setType(FrameEx.case_vide);
			}
		}
	}
